...
if __name__ == '__main__':
    config = patch_config.patch_configs['faster-rcnnmmdet3x']()
    model = config.model

    patch_path = "trained_patch/fasterrcnn_patch150_100_0.5_conf0.3_iou0.3_dlc.png"
    ...
    dota_target_cls_id = 0
    dior_target_cls_id = 2
    ...
    with open(asr_csv_path, "w", newline="", encoding="utf-8") as fcsv:
        ...
        for i_batch, (img_batch, lab_batch) in enumerate(train_loader):
            ...
            name = train_loader.dataset.img_names[i_batch].split('.')[0]
            ...
            img.save(f"patch_applier/images/{name}.png")

            # 推理 + NMS
            output = model.get_predictions(p_img_batch, [img_size, img_size], config.conf_thres)[0]
            ...
            if dota_target_cls_id is not None:
                keep = (cls_gt == float(dota_target_cls_id)) | (cls_gt == dota_target_cls_id)
                cls_gt = cls_gt[keep]
            ...
            det_boxes = output['bboxes']
            det_conf = output['scores']
            det_cls = output['labels']

            # 可选：只用某个类别的 detection 去匹配（比如 person=0）
            if dior_target_cls_id is not None:
                det_keep = (det_cls == float(dior_target_cls_id)) | (det_cls == dior_target_cls_id)
