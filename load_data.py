...
class MeanProbExtractor_yolo_ultralytics(nn.Module):
    """MaxProbExtractor: extracts max class probability for class from YOLO output.
    ...
    def __init__(self, cls_id, num_cls, loss_target, conf_thres, iou_thres, max_det, target_class):
        super(MeanProbExtractor_yolo_ultralytics, self).__init__()
        self.cls_id = cls_id
        self.num_cls = num_cls
        self.loss_target = loss_target
        self.conf_thres = conf_thres
    ...
    def forward(self, YOLOoutput):  # YOLOoutput: torch.Size([batch, 64512, 20])
        bboxes = YOLOoutput['bboxes']
        labels = YOLOoutput['labels']  # Extract label column, e.g. tensor([ 0.,  0.,  0.,  0., 14.])
        scores = YOLOoutput['scores']
        # print(output)
        index = labels == self.target_class  # Labels are plane, e.g. tensor([ True,  True,  True,  True, False])
        scores = scores[index]  # Predictions of plane
        # print(output)
        if len(scores) > 0:
            mean_conf = torch.mean(scores)  # The mean value of confs
        else:  # Without objects
            mean_conf = torch.tensor(float(0))
        # print(mean_conf)
        return mean_conf
...
class MeanProbExtractor_fasterrcnn_mmdet3x(nn.Module):
    """MaxProbExtractor: extracts max class probability for class from YOLO output.
    ...
    def __init__(self, cls_id, num_cls, loss_target, conf_thres, iou_thres, max_det, target_class):
        super(MeanProbExtractor_fasterrcnn_mmdet3x, self).__init__()
        self.cls_id = cls_id
        self.num_cls = num_cls
        self.loss_target = loss_target
        self.conf_thres = conf_thres
    ...
    def forward(self, Fasterrcnnoutput):  # YOLOoutput: torch.Size([batch, 64512, 20])
        bboxes = Fasterrcnnoutput['bboxes']
        labels = Fasterrcnnoutput['labels']  # Extract label column, e.g. tensor([ 0.,  0.,  0.,  0., 14.])
        scores = Fasterrcnnoutput['scores']
        # print(output)
        index = labels == self.target_class  # Labels are plane, e.g. tensor([ True,  True,  True,  True, False])
        scores = scores[index]  # Predictions of plane
        # print(output)
...
class PatchTransformer(nn.Module):
    ...
    def forward(self, adv_patch, lab_batch, img_size, do_rotate=True, rand_loc=True):
        ...
        target_size = torch.sqrt(
            ((lab_batch_scaled[:, :, 3].mul(self.patch_scale)) ** 2) + ((lab_batch_scaled[:, :, 4].mul(self.patch_scale)) ** 2))  # 0.2
