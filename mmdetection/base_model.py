...
class BaseModel(BaseModule):
    ...
    def attack_loss_step(self, data: Union[dict, tuple, list]) -> Tuple[Tensor, Dict[str, Tensor]]:
        """``BaseModel`` implements ``test_step`` the same as ``val_step``.
        ...
        """
        # result = []
        # parsed_losses = torch.tensor(0)
        # log_vars = {}

        data = self.data_preprocessor(data, True)
        losses = self._run_forward(data, mode='loss')
        parsed_losses, log_vars = self.parse_losses(losses[0])
        return parsed_losses, log_vars

    def attack_det_step(self, data: Union[dict, tuple, list]) -> Tuple[Any, Any]:
        """``BaseModel`` implements ``test_step`` the same as ``val_step``.
        ...
        ori_result = self._run_forward(data, mode='tensor')  # tuple,里面两个数组,一个cls[1000,81],一个bbox[1000,320]
        return ori_result[0][0], ori_result[0][1]

    def attack_result_step(self, data: Union[dict, tuple, list]) -> list:
        """``BaseModel`` implements ``test_step`` the same as ``val_step``.
        ...
        data = self.data_preprocessor(data, True)
        # data['inputs'] = data['inputs'][0]
        result = self._run_forward(data, mode='predict_attack')
        return result

    def test_step_gradcam(self, data: Union[dict, tuple, list]) -> list:
        """``BaseModel`` implements ``test_step`` the same as ``val_step``.
